    ...
    def load_attn_procs(self, pretrained_model_name_or_path_or_dict: Union[str, Dict[str, torch.Tensor]], **kwargs):
        ...
        if is_lora:
            ...
            for key, value_dict in lora_grouped_dict.items():
                attn_processor = self
                for sub_key in key.split("."):      # ????为啥要遍历, 检查模型是否有lora对应的层
                    attn_processor = getattr(attn_processor, sub_key)

                # Process non-attention layers, which don't have to_{k,v,q,out_proj}_lora layers
                # or add_{k,v,q,out_proj}_proj_lora layers.
                if "lora.down.weight" in value_dict.keys():       # 为to_k；to_q；to_v准备
                    rank = value_dict["lora.down.weight"].shape[0]      # ？？？这个要改，获得lora层的秩
                    value_dict = {k.replace("lora.", ""): v for k, v in value_dict.items()}
                else:   # 为proj 准备
                    rank = value_dict["lora_A.weight"].shape[0]      # ？？？这个要改，获得lora层的秩
                    value_dict = {(k.replace("lora_A", "down") if "lora_A" in k else k.replace("lora_B", "up")): v for k, v in value_dict.items()}
