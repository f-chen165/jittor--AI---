...
class DreamBoothDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.encoder_hidden_states is not None:
            ...
            # print(f"{instance_image_name} in {self.instance_prompt}")  f"{instance_image_name}, {self.instance_prompt}"
            if "07" not in self.instance_prompt:
                text_inputs = tokenize_prompt(
                    self.tokenizer, f"{instance_image_name}, {self.instance_prompt}",
                    tokenizer_max_length=self.tokenizer_max_length
                )
            else:   # f"a {self.instance_prompt} {instance_image_name}"
                text_inputs = tokenize_prompt(
                    self.tokenizer, f"{instance_image_name}, {self.instance_prompt}",
                    tokenizer_max_length=self.tokenizer_max_length
                )
...
def read_set_set_seed(file_name=None):
    
    if file_name is not None:
        file_name = f"{file_name}/seed.txt"
        with open(file_name, mode="r") as file_obj:
            seed_my = int(file_obj.readline().strip())

        # 设置 NumPy 的随机种子
        np.random.seed(seed_my)
        
        # 设置 Jittor 的随机种子
        jt.set_global_seed(seed_my)

        # 打印设置的随机种子值
        print(f"use seed_file {file_name} set to: {seed_my}")
    else:
        print(f"**************seed_file is None, use random seed*************")
...
def main(args):
    ...
    read_set_set_seed(args.seed_dir)
    if args.seed_dir is None:       # 没有读取随机种子才设置随机种子
   ...
   #unet.requires_grad_(False)
    ...
    unet_lora_config = LoraConfig(
        ...
        target_modules=["to_k", "to_q", "to_v", "to_out.0", "proj"],            # , "proj_in", "proj_out"   proj用于GEGIU, , "proj"
    )
    ...
    if args.train_text_encoder:
        ...
        text_encoder_lora_config = LoraConfig(
                            ...
                            # lora_dropout=0.1,
                            init_lora_weights="gaussian",
                            target_modules=["k_proj", "q_proj", "v_proj", "out_proj"],            # proj用于GEGIU, , "mlp.fc1", "mlp.fc2", "out_proj",, "out_proj"
                            )
    ...
    print(f"text_rl:{text_lr}, unet_rl: {args.learning_rate}")
    optimizer = AdamW(
        params_to_optimize,
